export default function FreshEs4() {
  ...
  const payments = [
    { month: "Diciembre 2024", date: "1 dic · A tiempo", amount: "$500", ok: true },
    { month: "Noviembre 2024", date: "30 oct · A tiempo", amount: "$500", ok: true },
    { month: "Octubre 2024", date: "2 oct · A tiempo", amount: "$500", ok: true },
    { month: "Septiembre 2024", date: "5 sep · Con retraso", amount: "$500", ok: false },
    { month: "Agosto 2024", date: "1 ago · A tiempo", amount: "$500", ok: true },
    { month: "Julio 2024", date: "1 jul · A tiempo", amount: "$500", ok: true },
  ];
  return (
    <div style={{ width: vw, height: vh, overflow: "hidden", position: "relative", background: bg }}>
      <div style={{ width: 428, height: innerH, transform: `scale(${scale})`, transformOrigin: "top left", fontFamily: "-apple-system,'SF Pro Display','Helvetica Neue',sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(160deg,#0d7a55 0%,#18a574 55%,#22c98a 100%)", flexShrink: 0 }}>
          <div style={{ padding: "8px 18px 0", display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 22, background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: "#fff" }}>JL</div>
            <div>
              <div style={{ fontSize: 18, fontWeight: 800, color: "#fff" }}>José López</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.75)" }}>Tanda Navidad · Turno 2/12</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, padding: "8px 18px 14px" }}>
            <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 18, padding: "4px 12px" }}><span style={{ fontSize: 12, fontWeight: 700, color: "#fff" }}>$500/turno</span></div>
            <div style={{ background: "rgba(240,165,0,0.35)", borderRadius: 18, padding: "4px 12px" }}><span style={{ fontSize: 12, fontWeight: 700, color: "#fff" }}>Pendiente</span></div>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: "hidden", padding: "12px 12px 0" }}>
          <div style={{ background: card, borderRadius: 14, padding: "14px", border: `1px solid ${border}`, marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: text, marginBottom: 12 }}>Registrar Pago</div>
            <div style={{ fontSize: 11, fontWeight: 600, color: muted, textTransform: "uppercase" as const, letterSpacing: "0.5px", marginBottom: 6 }}>Monto</div>
            <div style={{ display: "flex", alignItems: "center", background: bg, border: `2px solid ${primary}`, borderRadius: 10, padding: "10px 12px", marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 12, color: muted }}>MXN</span>
            </div>
            <div style={{ fontSize: 11, fontWeight: 600, color: muted, textTransform: "uppercase" as const, letterSpacing: "0.5px", marginBottom: 6 }}>Fecha de pago</div>
            <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 10, padding: "10px 12px", marginBottom: 12, display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 14, color: text }}>12 de enero, 2025</span>
              <span>📅</span>
            </div>
            <div style={{ background: "linear-gradient(135deg,#0d7a55,#18a574)", borderRadius: 10, padding: "13px", textAlign: "center" }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>✓ Confirmar Pago</span>
            </div>
          </div>
          <div style={{ fontSize: 11, fontWeight: 700, color: muted, textTransform: "uppercase" as const, letterSpacing: "0.5px", marginBottom: 8 }}>Historial de Pagos</div>
          {payments.map((p,i)=>(
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top,rgba(4,18,10,0.93) 0%,rgba(4,18,10,0.75) 50%,transparent 100%)", padding: `${Math.round(vh*0.07)}px ${Math.round(vw*0.06)}px ${Math.round(vh*0.04)}px`, textAlign: "center" }}>
        <div style={{ fontSize: Math.round(vw*0.074), fontWeight: 800, color: "#fff", letterSpacing: -1, lineHeight: 1.15 }}>Registra pagos<br/>en segundos</div>
        <div style={{ fontSize: Math.round(vw*0.033), color: "rgba(255,255,255,0.65)", marginTop: Math.round(vh*0.008), fontWeight: 500 }}>Historial completo de cada integrante</div>
      </div>
    </div>
  );
}
