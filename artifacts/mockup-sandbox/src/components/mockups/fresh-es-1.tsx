  ...
  return (
    <div style={{ width: vw, height: vh, overflow: "hidden", position: "relative", background: bg }}>
      <div style={{ width: 428, height: innerH, transform: `scale(${scale})`, transformOrigin: "top left", fontFamily: "-apple-system,'SF Pro Display','Helvetica Neue',sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ background: "linear-gradient(160deg,#0d7a55 0%,#18a574 55%,#22c98a 100%)", flexShrink: 0 }}>
          <div style={{ padding: "14px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ position: "absolute", left: "50%", transform: "translateX(-50%)", width: 120, height: 34, background: "#000", borderRadius: 20 }} />
          ...
          <div style={{ padding: "10px 18px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontSize: 26, fontWeight: 800, color: "#fff", letterSpacing: -0.5 }}>Mis Tandas</div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,0.7)" }}>6 tandas · 56 integrantes</div>
            </div>
          ...
          <div style={{ display: "flex", gap: 7, padding: "10px 16px 14px" }}>
            {[["5 Activas","rgba(255,255,255,0.18)"],["$8,450 Cobrado","rgba(240,165,0,0.3)"]].map(([label,bg2],i)=>(
          ...
          {circles.map((c,i)=>(
            <div key={i} style={{ background: card, borderRadius: 14, padding: "12px 13px", marginBottom: 8, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: `1px solid ${border}` }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div><div style={{ fontSize: 10, color: muted }}>Aportación</div><div style={{ fontSize: 16, fontWeight: 800, color: primary }}>${c.amount.toLocaleString()}</div></div>
                <div style={{ textAlign: "center" }}><div style={{ fontSize: 10, color: muted }}>Turno</div><div style={{ fontSize: 13, fontWeight: 700, color: text }}>{c.cycle}/{c.total}</div></div>
                <div style={{ textAlign: "right" }}><div style={{ fontSize: 10, color: muted }}>% Completado</div><div style={{ fontSize: 13, fontWeight: 700, color: primary }}>{c.pct}%</div></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top,rgba(4,18,10,0.93) 0%,rgba(4,18,10,0.75) 50%,transparent 100%)", padding: `${Math.round(vh*0.07)}px ${Math.round(vw*0.06)}px ${Math.round(vh*0.04)}px`, textAlign: "center" }}>
        <div style={{ fontSize: Math.round(vw*0.074), fontWeight: 800, color: "#fff", letterSpacing: -1, lineHeight: 1.15 }}>Todas tus tandas<br/>en un solo lugar</div>
        <div style={{ fontSize: Math.round(vw*0.033), color: "rgba(255,255,255,0.65)", marginTop: Math.round(vh*0.008), fontWeight: 500 }}>Controla cada círculo de ahorro fácilmente</div>
      </div>
    </div>
  );
}
