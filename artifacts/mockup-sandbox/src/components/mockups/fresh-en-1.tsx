export default function FreshEn1() {
  ...
  const circles = [
    ...
    { name: "Community Savings", freq: "Monthly", active: true, cycle: 4, total: 10, amount: 200, members: 10, pct: 40 },
    ...
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", background: bg, fontFamily: "-apple-system,'SF Pro Display','Helvetica Neue',sans-serif", display: "flex", flexDirection: "column" }}>
      <div style={{ background: "linear-gradient(160deg,#0d7a55 0%,#18a574 55%,#22c98a 100%)", flexShrink: 0, paddingBottom: 16 }}>
        ...
        <div style={{ padding: "14px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontSize: 30, fontWeight: 800, color: "#fff", letterSpacing: -0.5 }}>My Circles</div>
            <div style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", marginTop: 2 }}>7 circles · 54 members</div>
          </div>
        ...
        <div style={{ display: "flex", gap: 8, padding: "12px 18px 0" }}>
          {[["6 Active","rgba(255,255,255,0.18)"],["$8,450 Collected","rgba(240,165,0,0.3)"],["54 Members","rgba(255,255,255,0.12)"]].map(([label,bg2],i)=>(
          ...
          <div key={i} style={{ background: card, borderRadius: 16, padding: "13px 15px", marginBottom: 9, boxShadow: "0 2px 6px rgba(0,0,0,0.06)", border: `1px solid ${border}` }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 7 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: muted, marginTop: 2 }}>↻ {c.freq} · {c.members} members</div>
              </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div><div style={{ fontSize: 11, color: muted }}>Contribution</div><div style={{ fontSize: 18, fontWeight: 800, color: primary }}>${c.amount.toLocaleString()}</div></div>
              <div style={{ textAlign: "center" }}><div style={{ fontSize: 11, color: muted }}>Turn</div><div style={{ fontSize: 15, fontWeight: 700, color: text }}>{c.cycle}/{c.total}</div></div>
              <div style={{ textAlign: "right" }}><div style={{ fontSize: 11, color: muted }}>Progress</div><div style={{ fontSize: 15, fontWeight: 700, color: primary }}>{c.pct}%</div></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(to top,rgba(4,18,10,0.93) 0%,rgba(4,18,10,0.75) 50%,transparent 100%)", padding: "110px 32px 52px", textAlign: "center" }}>
        <div style={{ fontSize: 48, fontWeight: 900, color: "#fff", letterSpacing: -1, lineHeight: 1.1 }}>All your circles<br/>in one place</div>
        <div style={{ fontSize: 20, color: "rgba(255,255,255,0.65)", marginTop: 10, fontWeight: 500 }}>Manage every savings circle with ease</div>
      </div>
    </div>
  );
}
