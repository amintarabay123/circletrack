export default function CircleDetailEnPreview() {
  ...
  const members = [
    { name: "Maria Garcia", paid: true, late: false, due: 500, paid_amt: 500 },
    { name: "Jose Lopez", paid: false, late: false, due: 500, paid_amt: 0 },
    { name: "Ana Martinez", paid: false, late: true, due: 500, paid_amt: 0 },
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center", color: "#fff", zIndex: 1 }}>
        ...
        <div style={{ fontSize: 32, color: "rgba(255,255,255,0.55)", fontWeight: 400 }}>
          Live dashboard for every cycle
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ padding: "8px 36px 8px", display: "flex", alignItems: "center" }}>
          ...
          <div style={{ flex: 1, marginLeft: 20 }}>
            <div style={{ fontSize: 34, fontWeight: 700, color: text }}>Christmas Circle 2025</div>
            <div style={{ fontSize: 20, color: muted }}>Monthly · Cycle 3 of 12</div>
          </div>
        </div>

        <div style={{ display: "flex", borderBottom: `2px solid ${border}`, marginLeft: 36, marginRight: 36 }}>
          <div style={{ padding: "16px 28px", borderBottom: `3px solid ${primary}`, color: primary, fontSize: 26, fontWeight: 600 }}>Summary</div>
          <div style={{ padding: "16px 28px", color: muted, fontSize: 26 }}>Members</div>
        </div>
        ...
        <div style={{ padding: "20px 36px" }}>
          <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
            <div style={{ flex: 1, background: card, borderRadius: 22, padding: "22px 26px", border: `1px solid ${border}`, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 18, color: muted, marginTop: 4 }}>Pot amount</div>
            </div>
            <div style={{ flex: 1, background: card, borderRadius: 22, padding: "22px 26px", border: `1px solid ${border}`, textAlign: "center" }}>
              <div style={{ fontSize: 42, fontWeight: 700, color: success }}>75%</div>
              <div style={{ fontSize: 18, color: muted, marginTop: 4 }}>Collection rate</div>
            </div>
          </div>

          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            {[["2", "Paid", success], ["1", "Unpaid", muted], ["1", "Late", danger]].map(([v, l, c], i) => (
          ...
          <div style={{ display: "flex", alignItems: "center", background: primary + "12", borderRadius: 22, padding: "18px 24px", marginBottom: 20, border: `1px solid ${primary}28` }}>
            ...
            <div>
              <div style={{ fontSize: 18, color: muted }}>Pot recipient</div>
              <div style={{ fontSize: 28, fontWeight: 600, color: primary }}>Maria Garcia</div>
            </div>
          </div>

          <div style={{ fontSize: 20, fontWeight: 700, color: muted, textTransform: "uppercase", letterSpacing: "1px", marginBottom: 14 }}>Payment Status</div>
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", background: card, borderRadius: 22, padding: "18px 22px", marginBottom: 14, border: `1px solid ${border}`, gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
                <span style={{ fontSize: 18, fontWeight: 700, padding: "5px 14px", borderRadius: 40, color: m.paid ? success : m.late ? danger : muted, background: m.paid ? success + "18" : m.late ? danger + "18" : muted + "18" }}>
                  {m.paid ? "Paid" : m.late ? "Late" : "Unpaid"}
                </span>
                {!m.paid && (
                  <div style={{ background: primary, color: "#fff", fontSize: 18, fontWeight: 700, padding: "6px 16px", borderRadius: 12 }}>+ Record</div>
                )}
              </div>
            </div>
          ))}
