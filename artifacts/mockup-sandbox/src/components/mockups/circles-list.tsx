    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: "14px 44px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontSize: 52, fontWeight: 700, color: text }}>Mis Tandas</span>
          ...
          {circles.map((c, i) => (
            <div key={i} style={{ background: card, borderRadius: 28, padding: "26px 30px", marginBottom: 20, border: `1px solid ${border}` }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 20, fontWeight: 700, color: c.active ? primary : muted, background: c.active ? primary + "18" : "#eff1f5", padding: "6px 18px", borderRadius: 40, letterSpacing: 0.5 }}>
                  {c.active ? "ACTIVA" : "INACTIVA"}
                </span>
              </div>
              <div style={{ display: "flex", gap: 20, marginTop: 12 }}>
                <span style={{ fontSize: 22, color: muted }}>↻ {c.freq}</span>
                <span style={{ fontSize: 22, color: muted }}>Turno {c.cycle}/{c.total}</span>
              </div>
              <div style={{ height: 1, background: border, margin: "16px 0" }} />
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 40, fontWeight: 700, color: primary }}>${c.amount.toLocaleString()}</span>
                <span style={{ fontSize: 22, color: muted }}>Turno {c.cycle} de {c.total} ›</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: card, borderTop: `1px solid ${border}`, display: "flex", paddingBottom: 40 }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 20, gap: 6 }}>
            ...
            <span style={{ fontSize: 20, fontWeight: 700, color: primary }}>Tandas</span>
          </div>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", paddingTop: 20, gap: 6 }}>
            ...
            <span style={{ fontSize: 20, color: muted }}>Ajustes</span>
          </div>
        </div>
      </div>
    </div>
